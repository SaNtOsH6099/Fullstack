# JavaScript DOM and Event Handling(Task 18)

JavaScript DOM and Event Handling

Click the link below to see the live deployement
[https://santosh6099.github.io/Fullstack/Html-Css-Js/Task-18/](https://santosh6099.github.io/Fullstack/Html-Css-Js/Task-18/)

## features

- proper comments.
- no redundancy only single eventlistener for all boxes
- toggle color on or off.
- realtime input check to revert the header

# How to run

1. Download all the files.
2. Make a Folder and move the downloaded files into the folder.
3. Download Live Server extension.
4. open the "index.html" file and run the live server located at the bottom right on your Screen.
5. Right click on the page and select "Inspect" to view the results in the console.
